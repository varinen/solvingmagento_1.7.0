...
$header_array = array(
    ...
    'has_options', 'is_in_stock', 'tax_class_id', 'visibility', 'description',
    'short_description', 'qty', 'color'
);
 ...
 */
function create_simple($counter, $sku=false, $price=false, $cats=false, $gem=false, $color=false) {
    $type = 'simple';
    ...
    $line_array = array(
        ...
        '', '', 0, 1, 1, $visibility, 'description', 'short_description', 1000,
        $color
    );
    // write to file
...
function create_configurable($counter) {
    ...
    for (; $i <= $count_simples; $i++) {
        ...
        create_simple($i, $simple_sku, $price, $cats);
    }

    $counter += $i;
    $simples_skus = implode(',', $simple_skus);
    $sku = "{$sku}_cfg";
    $conf_attributes = implode(',', array('color'));
    $cats = implode(',', $cats);

    // CSV array
    $line_array = array(
        'admin', 'base', 'default', $cats, $sku, $name, $price, 1, $simples_skus,
        $type, $conf_attributes, '', 1, 1, 1, 4, 'description',
